...
// In that case, the previous example would be written as:
  ...
  // Now if the customer is too young to be granted access to our bar, we can at least explain the reason with a message!
   ...
   // There are cases when executing either one piece of code or the other would not suffice.
...
// Had we not use an elseif statement but two if statements in a row:
  $age = 19;
   ...
   // Now we get two messages in a row. This is because PHP checks both if statements. Even if the first statement has been evaluated to true it won't stop.
...
// Regarding the previous example, we could remove nesting with the use of elseif statements as some small changes in the conditional statements.
...
// Our code now is a little cleaner, but not necessarily more readable. Readability has a lot to do with the context.
...
// OUtcome: Parse error: syntax error, unexpected token "if", expecting ":" in Standard input code on line 6
...
//          Well Done!"
/*
  If the grade is “A+” (Line 15), the program executes the next statement until it reaches the break statement. This means it’ll execute lines 16 to 19.

If the grade is “A” (Line 16), the program executes lines 17 to 19.  
